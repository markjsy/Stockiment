...
# StockTwits details
os.environ['ST_ACCESS_TOKEN'] = 'a02e36799d561ef8f7eb5343e0bf4f9556429b80'
...
def get_watchlist_ids():
    """ Get the list of watchlist ids from the user
    """
    wl_id = get_json(ST_BASE_URL + "watchlists.json", params=ST_BASE_PARAMS)
    return wl_id

def get_watched_stocks(wl_id):
    """ Get list of symbols being watched by specified StockTwits watchlist
    """
    wl = get_json(ST_BASE_URL + 'watchlists/show/{}.json'.format(wl_id), params=ST_BASE_PARAMS)
    return wl
...
def get_message_stream(wl_id, params={}):
    ...
    for k, v in params.iteritems():
        all_params[k] = v
    return get_json(ST_BASE_URL + 'streams/watchlist/{}.json'.format(wl_id), params=all_params)
...
def add_to_watchlist(symbols, wl_id):
    ...
    resp = post_json(ST_BASE_URL + 'watchlists/{}/symbols/create.json'.format(wl_id), params=params, deadline=deadline)
...
def delete_from_watchlist(symbol, wl_id):
    ...
    resp = post_json(ST_BASE_URL + 'watchlists/{}/symbols/destroy.json'.format(wl_id), params=params)
...
def clean_watchlist(wl_id):
    ...
    """
    wl = get_json(ST_BASE_URL + 'watchlists/show/{}.json'.format(wl_id),
